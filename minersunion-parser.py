...
def fetch_subnets(timeout=10):
    """Return list of all subnet objects."""
...
def fetch_history(netuid, timeout=10):
    """Fetch history (validators) for a given subnet by netuid."""
...
def main():
    # 1. Получаем список подсетей и строим карту netuid → subnet_name
    ...
    for netuid in range(1, max_netuid + 1):
        ...
        # Для каждой записи в истории добавляем строку
        for v in history:
            rows.append([
                netuid,
                subnet_name,
                ...
                v.get('hotkey', ''),
                v.get('total_stake', ''),
                v.get('weighted_div_vtrust_score', ''),
                v.get('dividends', ''),
                v.get('chk_take', '')
            ])

    # 4. Записываем всё в Google Sheets
    sheet.clear()
